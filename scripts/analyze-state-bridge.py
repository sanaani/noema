...
def load_centroids(vectors, index):
    z = np.load(vectors)
    ...
    for line in gzip.open(index, "rt"):
        ...
        c = V[r["text_indices"]].mean(axis=0)
        centroid[name] = c / np.linalg.norm(c)
        kind[name] = r["object_kind"]
        count[name] = len(r["text_indices"])
    return centroid, kind, count, len(texts)
...
def replication(names, X, rare, mod, permutations, rng, note):
    sim = X @ X.T
    ...
    perm = (draws - pos * (pos + 1) / 2) / (pos * neg)
    p = float((np.sum(perm >= observed) + 1) / (len(perm) + 1))
...
def betweenness(centroid, kind, names, X):
...
def main():
    ...
    centroid, kind, count, n_texts = load_centroids(args.vectors, args.index)
    ...
    report = {"objects": len(names), "texts": n_texts}
    report["replication_all"] = replication(names, X, rare, mod, args.permutations, rng, "all objects")
