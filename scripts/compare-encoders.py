...
def prepare():
    if any((OUT / name / "execution.json").exists() for name in MODELS):
        raise ValueError("completed model outputs exist; prepare a new --output directory")
    OUT.mkdir(parents=True, exist_ok=True)
    if OUT.resolve() != DEFAULT_OUT.resolve():
        for name in ("Fixtures.lean", "Controls.lean", "protocol.md"):
            shutil.copyfile(DEFAULT_OUT / name, OUT / name)
...
def seal():
    if not (OUT / "summary.json").exists():
        raise ValueError("summarize completed models before sealing")
...
if __name__ == "__main__":
    ...
    parser.add_argument("stage", choices=["prepare", "encode", "summarize", "verify", "seal"])
    ...
    if args.stage == "encode":
        ...
        {"prepare": prepare, "summarize": summarize, "verify": verify, "seal": seal}[args.stage]()
