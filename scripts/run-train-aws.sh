#!/bin/bash
# Train phase 4's two self-supervised encoders on a temporary AWS GPU worker,
# and embed the union with them.
#
# The worker checks the frozen inputs against their manifest, runs
# scripts/train-encoders-gpu.py (every setting fixed in the phase 4 README),
# uploads the result and powers off; the instance terminates on shutdown and
# a 3-hour timer backs that up. No label is packaged: only the texts and the
# tactic heads go up.
#
# Roughly $2-5: a g6e.xlarge at ~$1.86/hr, about an hour of training.
#
#   scripts/run-train-aws.sh launch   <inputs-dir>
#   scripts/run-train-aws.sh watch    <run-name>
#   scripts/run-train-aws.sh fetch    <run-name>
#   scripts/run-train-aws.sh teardown <run-name>
set -euo pipefail
...
OUTROOT="${OUTROOT:-$REPO/outputs/phase-4-trained-encoder}"

launch() {
  local inputs=${1:?path to the inputs directory}
  local run ts
  ...
  cp "$inputs/texts.json.gz" "$inputs/tactic-heads.jsonl.gz" "$inputs/inputs-manifest.json" \
     "$out/task/noema/in/"
  cp "$REPO/scripts/train-encoders-gpu.py" "$out/task/noema/scripts/"
...
JSON
  aws iam create-role --role-name "$run" \
    ...
    --tags Key=Project,Value=noema Key=Purpose,Value=temporary-encoder-training >/dev/null
  ...
  sg=$(aws ec2 create-security-group --region "$REGION" --group-name "$run" \
        --description "temporary noema training worker" --vpc-id "$vpc" \
  ...
  sed -e "s|@BUCKET@|$BUCKET|g" -e "s|@RUN@|$run|g" \
      "$REPO/scripts/train-user-data.sh.in" > "$out/user-data.sh"
  ...
  for itype in $INSTANCE_TYPES; do
   ...
   id=$(aws ec2 run-instances --region "$REGION" \
    ...
    --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=$run},{Key=Project,Value=noema},{Key=Purpose,Value=temporary-encoder-training}]" \
  ...
  echo "artifacts $out"
  echo
  echo "watch:    scripts/run-train-aws.sh watch $run"
}
...
watch_run() {
  ...
  aws ec2 get-console-output --region "$REGION" --instance-id "$id" --latest \
    ...
    | grep -E "(boot|pydeps-done|STAGE|epoch|JOB DONE|FINISH|NVIDIA|L40S|Error)" | tail -8
  echo "s3:"
...
case "${1:-}" in
  launch)   launch "${2:?path to the inputs directory}" ;;
