#!/bin/bash
# Print 2026 Mathlib statements for phase 5, on a temporary AWS CPU worker.
#
# results/phase-5-conjecture-map/Statements2026.lean prints each name in the
# committed draw (draw/names.txt: connectors, the comparison sample, surviving
# corpus theorems) from Mathlib 09712d48 with Lean v4.35.0-rc2. Importing the
# 5,340 modules needs more memory than a laptop has, so it runs here, on the
...
#
#   scripts/run-statements-2026-aws.sh launch
#   scripts/run-statements-2026-aws.sh watch    <run-name>
#   scripts/run-statements-2026-aws.sh fetch    <run-name>
#   scripts/run-statements-2026-aws.sh teardown <run-name>
set -euo pipefail
...
launch() {
  ...
  run="noema-stmt2026-$ts"
  local out="$REPO/outputs/phase-5-conjecture-map/$run"
  mkdir -p "$out"

  echo "== packaging the printer and the draw"
  local P5="$REPO/results/phase-5-conjecture-map"
  tar czf "$out/task.tar.gz" -C "$P5" Statements2026.lean draw/names.txt draw/modules.txt
...
JSON
  ...
  sed -e "s|@BUCKET@|$BUCKET|g" -e "s|@RUN@|$run|g" \
      ...
      "$REPO/scripts/statements-2026-user-data.sh.in" > "$out/user-data.sh"
  ...
  echo "artifacts $out"
  echo
  echo "watch:    scripts/run-statements-2026-aws.sh watch $run"
}

watch_run() {
  local run=$1 out="$REPO/outputs/phase-5-conjecture-map/$1"
