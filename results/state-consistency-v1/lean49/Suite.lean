...
elab "state " label:str " : " ty:term : command => liftTermElabM do
  let ty ← Term.withDeclName (Name.mkSimple ("noema_" ++ label.getString)) (Term.elabType ty)
...
elab "audit_fixture " name:ident : command => do
  ...
  let (_, axiomState) := ((CollectAxioms.collect name.getId).run (← getEnv)).run {}
  let axioms := axiomState.axioms
  for a in axioms do
