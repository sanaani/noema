...
((p0) ∧ (p1) ∧ (p1 → p2) ∧ ((p0 ∧ p1) → p2) ∧ (p0 → p2) ∧ (p1 → p3) ∧ ((p0 ∧ p1) → p3) ∧ (p0 → p3) ∧ ((p0 ∧ p2) → p4) ∧ ((p0 ∧ p1) → p4) ∧ ((p1 ∧ p2) → p4) ∧ (p1 → p5) ∧ (p2 → p5) ∧ ((p1 ∧ p4) → p5) ∧ (p5 → p6) ∧ ((p1 ∧ p2) → p6) ∧ (p0 → p6) ∧ ((p1 ∧ p3) → p7) ∧ ((p3 ∧ p4) → p7) ∧ ((p0 ∧ p4) → p7)) ↔ ((p0) ∧ (p1) ∧ (p2) ∧ (p3) ∧ (p4) ∧ (p5) ∧ (p6) ∧ (p7)) := by
  constructor
  · intro h
    rcases h with ⟨h0, h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12, h13, h14, h15, h16, h17, h18, h19⟩
    exact ⟨h0, ⟨h1, ⟨(h4 h0), ⟨(h7 h0), ⟨(h10 ⟨h1, (h3 ⟨h0, h1⟩)⟩), ⟨(h12 (h3 ⟨h0, h1⟩)), ⟨(h15 ⟨h1, (h3 ⟨h0, h1⟩)⟩), (h18 ⟨(h6 ⟨h0, h1⟩), (h8 ⟨h0, (h2 h1)⟩)⟩)⟩⟩⟩⟩⟩⟩⟩
  · intro h
